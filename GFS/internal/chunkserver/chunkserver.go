...
func (cs *ChunkServer) startHeartbeat() {
	for {
		...
		cs.runHeartbeatSession()

		// Back off before reconnecting so we don't hammer a restarting master
		select {
		case <-cs.heartbeatStop:
			return
...
// runHeartbeatSession opens one HeartBeat stream and pumps it until it breaks.
// On return the caller will reconnect (and re-run reportChunks so the master
// repopulates chunk locations after a restart).
func (cs *ChunkServer) runHeartbeatSession() {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()

	stream, err := cs.masterClient.HeartBeat(ctx)
	if err != nil {
		log.Printf("Failed to start heartbeat stream: %v", err)
		return
	}

	// Re-report all local chunks so the master rebuilds Locations after restart
	go func() {
		if err := cs.reportChunks(); err != nil {
			log.Printf("Failed to report chunks on reconnect: %v", err)
		}
	}()
	...
	for {
		...
		case <-ticker.C:
			...
			if err := stream.Send(req); err != nil {
				log.Printf("Heartbeat send failed, will reconnect: %v", err)
				return
			}
			resp, err := stream.Recv()
			if err != nil {
				log.Printf("Heartbeat recv failed, will reconnect: %v", err)
				return
			}
